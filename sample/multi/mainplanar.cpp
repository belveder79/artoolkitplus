...
void fillMarkerId(int startid, int wid, int height, ARToolKitPlus::ARMultiEachMarkerInfoT* marker)
{
  int i = 0; int runningid = startid;
  for(int r = 0; r < height; r++) // -2 -1 0 1
    for(int c = 0; c < wid; c++, runningid++, i++) // -2 -1 0 1
    {
      marker[i].patt_id = runningid; // 0 works!
